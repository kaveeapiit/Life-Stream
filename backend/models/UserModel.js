...
export async function listUsers({
  search = '', limit = 20, offset = 0, sort = 'created_at', dir = 'DESC'
}) {
...
export async function createUserRow(data) {
  const body = Object.fromEntries(Object.entries(data).filter(([k, v]) => ALLOW.includes(k) && v !== undefined));
  if (body.password) {
...
export async function updateUserRow(id, data) {
  ...
  const { rows } = await pool.query(
    `UPDATE ${TABLE} SET ${clause} WHERE id=$${values.length + 1} RETURNING id, email, name, blood_type AS blood_group, created_at`,
    [...values, id]
  );
  return rows[0];
}
