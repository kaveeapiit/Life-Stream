...
export default function Login() {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white overflow-hidden px-4 sm:px-6 lg:px-8">
      ...
      {/* card */}
      <div className="relative w-full max-w-sm sm:max-w-md lg:max-w-lg backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 sm:p-8 lg:p-10 shadow-2xl animate-fadeIn">
        {/* logo + title */}
        ...
        {/* error */}
        {err && (
          <div className="mb-4 px-3 sm:px-4 py-2 rounded-md bg-red-500/20 border border-red-500/40 text-red-200 text-sm">
            {err}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-3 top-[46px] sm:top-[52px] -translate-y-1/2 text-gray-400 hover:text-gray-200 p-1 touch-manipulation"
              aria-label={showPw ? 'Hide password' : 'Show password'}
            >
              {showPw ? <FaEyeSlash /> : <FaEye />}
            </button>
            <div className="text-right mt-1">
              <a href="#" className="text-xs sm:text-sm text-red-300 hover:underline">Forgot Password?</a>
            </div>
          </div>
...
function StaticInput({ icon, label, value, name, type = 'text', onChange, required }) {
  return (
    <div>
      <label htmlFor={name} className="block mb-1 text-sm sm:text-base font-semibold text-gray-300">{label}</label>
      <div className="relative">
        <span className="absolute top-1/2 -translate-y-1/2 left-3 text-sm sm:text-base">{icon}</span>
        <input
          id={name}
          type={type}
          name={name}
          ...
          required={required}
          placeholder={label}
          className="w-full pl-10 pr-4 py-2.5 sm:py-3 rounded-lg bg-gray-900/50 border border-gray-700
                     text-sm sm:text-base font-semibold text-white placeholder-gray-400
